<template>
  <div class="row align-items-center justify-content-center mx-1">
    <div
      ...
      class="mt-4 col-12 px-0 cardCollection-image"
    >
      <div v-if="card.image_uris?.normal">
        <img
          class="img-fluid borderRadius shadow cardsBg"
          :class="
            !activeDeck.id ||
            deckCard?.find((d) => d.card.cardId == card.cardId)
              ? ''
              : 'card-in-deck'
          "
  ...
  <div class="col-9 mx-auto">
    <div class="row">
      <div
        class="col-5 deckText xsFont align-items-center d-flex justify-content-center"
      >
        <div class="row">
          <p class="p-0 m-0">
            <i class="mdi numberBox p-0 m-0 mdi-card-multiple-outline"></i>&nbsp
            {{ card.count }}
          </p>
        </div>
      </div>
      <div class="col-3">
        <i
          @click.prevent="removeCard() && getAccountCards()"
          class="deckText mdi mdi-minus-circle text-light"
          value="Delete"
          type="button"
        >
        </i>
      ...
      <div class="col-3">
        <i
          v-if="activeDeck"
          @click="createDeckCard(card)"
          title="Add to Deck"
          class="deckText mdi mdi-plus-circle mdi"
          type="button"
        ></i>
      </div>
    </div>
    ...
    return {
      ...
      async removeCard() {
        try {
          ...
          logger.log("Removed Card Id:", cardId);
