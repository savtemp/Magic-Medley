<template>
  ...
  <div @click="getCardById" class="row position-relative">
    <div
      class="col-10 offset-1 magicCard text-start border p-2 mt-1 rounded"
    ...
    >
      ...
      <span
        class="magicCard"
        :title="oracleCard.set_name + ' ' + oracleCard.name"
        v-if="oracleCard.prices.usd"
        ><a
          target="_blank"
          rel="noopener noreferrer"
          :href="oracleCard.purchase_uris.tcgplayer"
          >${{ oracleCard.prices.usd }}</a
        ></span
      >
...
export default {
  ...
  setup(props) {
    return {
      ...
      async getCardById() {
        try {
          logger.log("Props ID:", props.oracleCard);
